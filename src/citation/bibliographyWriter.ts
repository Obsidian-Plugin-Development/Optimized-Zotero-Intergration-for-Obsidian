...
export function initBibEmitter(emitter: any) {
	...
	DEBUG_BIB_WRITER && console.log("[BibWriter Debug] pluginEmitter 已初始化");
}
...
function setBibDirty(dirty: boolean, silent = false) {
	DEBUG_BIB_WRITER && console.log("[BibWriter Debug] setBibDirty 被调用, dirty =", dirty, "当前 isBibOutOfSync =", isBibOutOfSync, "dirtyCallbacks 数量 =", dirtyCallbacks.length);
	if (isBibOutOfSync === dirty) {
		DEBUG_BIB_WRITER && console.log("[BibWriter Debug] 状态未变化，提前返回");
		return;
	}
	isBibOutOfSync = dirty;
	DEBUG_BIB_WRITER && console.log("[BibWriter Debug] 开始调用", dirtyCallbacks.length, "个回调");
	for (const cb of dirtyCallbacks) {
		try { cb(dirty); } catch { /* 静默 */ }
	}
	DEBUG_BIB_WRITER && console.log("[BibWriter Debug] 回调调用完成");

	// v6.5.4: 直接触发 plugin.emitter 事件，传递最新文档内容避免从磁盘读取旧内容
	if (!silent && pluginEmitter) {
		DEBUG_BIB_WRITER && console.log("[BibWriter Debug] 直接触发 plugin.emitter 事件:", dirty ? "bibDirty" : "bibClean");
		try {
